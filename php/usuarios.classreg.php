...
//$sucbx=$_SESSION['codsuc'];		
		include('../php/conexion.php');
        $datos = array();
		$resultado = mysqli_query($conexion,"SELECT * FROM articulos WHERE descripar LIKE '%$nombreUsuario%' ");
        ...
        while ($row = mysqli_fetch_array($resultado, MYSQLI_ASSOC)){
            $datos[] = array("value" => $row['descripar'],
                             ...
                             "codpvx" => $row['codpv'],
                              ...
                              "modelox" => $row['modelo'],
                              "seriex" => $row['serie'],
                              "procex" => $row['procedencia'],
                              "prevtax" => number_format($row['pneto']+($row['pneto']*0.30),2),
							 "prefacx" =>number_format((number_format($row['pneto']+($row['pneto']*0.30),2))+((number_format($row['pneto']+($row['pneto']*0.30),2))*(0.13/0.87)),2),
							 "gananx" => number_format(($row['pneto']+($row['pneto']*0.30))-$row['pneto'],2),
							 "tasax" => number_format((0.13/0.87),9),
                             "fotoarx" => $row['fotoar']);
        }
