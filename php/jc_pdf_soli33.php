...
$row = mysqli_query($conexion,"SELECT * FROM solicitudtot2 AS so LEFT JOIN areas AS ar ON so.codarea=ar.codarea LEFT JOIN cargo AS ca ON so.codcargo=ca.codcargo WHERE so.norden='$norden'");
...
$html .= '<table border="1" width="100%">';
$html .= '<tr><td align="center"><h4>FORM.NOTA DE ENTREGA AL ALMACEN DE MANTENIMIENTO</h4></td></tr>';
$html .= '</table>';

$html .= '<table border="1" width="100%">';
$html .= '<tr><td>Nombre:</td><td>'.$afavorx.'</td><td align="right">Fecha Solicitud :</td><td>'.$fe2.'</td></tr>';
$html .= '<tr><td>Area :</td><td>'.$detarea.'</td><td align="right">No de Solicitud :</td><td>'.$nordenx.'</td></tr>';
