...
$area=$_SESSION['id_area'];
    ...
    $nroProductos = mysqli_num_rows(mysqli_query($conexion,"SELECT * FROM articulos AS ar LEFT JOIN clasificacion AS cl ON ar.codcla=cl.codcla LEFT JOIN proveedor AS pv ON ar.codpv=pv.codpv LEFT JOIN articulossuc AS ars ON ar.codar=ars.codar WHERE  ars.codsuc='$codsucx' "));
    $nroLotes = 10;
    ...
    if($paginaActual < $nroPaginas){
  	...
  	$registro = mysqli_query($conexion,"SELECT * FROM articulos AS ar LEFT JOIN clasificacion AS cl ON ar.codcla=cl.codcla LEFT JOIN proveedor AS pv ON ar.codpv=pv.codpv LEFT JOIN articulossuc AS ars ON ar.codar=ars.codar WHERE  ars.codsuc='$codsucx'  LIMIT $limit, $nroLotes ");
  	...
  	$tabla = $tabla.'<table class="table table-striped table-condensed table-hover table-bordered titi">
			            <tr>
			                ...
			                <td width="12%" align="center">Proveedor</td>
