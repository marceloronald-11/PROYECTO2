...
$row = mysqli_query($conexion,"SELECT * FROM solicitudtot2 AS so LEFT JOIN areas AS ar ON so.codarea=ar.codarea LEFT JOIN cargo AS ca ON so.codcargo=ca.codcargo LEFT JOIN solicita AS sl ON so.codsoli=sl.codsoli WHERE so.norden='$norden'");
...
$idusux=$row2['id_usu'];
$nomsolicita=$row2['detsolicitante'];
...
// $row1 = mysqli_query($conexion,"SELECT * FROM usuarios WHERE id_usu='$idusux'");
// while ($row3 = mysqli_fetch_array($row1)) {
//    $codperx=$row3['codper'];
// }

// $row4 = mysqli_query($conexion,"SELECT * FROM personal AS pe LEFT JOIN cargo AS ca ON pe.codcargo=ca.codcargo LEFT JOIN areas AS ar ON pe.codarea=ar.codarea  WHERE pe.codper='$codperx'");
// $dtcargo='';
// $dtarea='';
// while ($row6 = mysqli_fetch_array($row4)) {
//    $dtcargo=$row6['detcargo'];
//    $dtarea=$row6['detarea'];
// }
...
$html .= '<table border="1" width="100%">';
$html .= '<tr><td align="center"><h4>SOLICITUD DE REPUESTOS Y MATERIALES</h4></td></tr>';
$html .= '</table>';

$html .= '<table border="1" width="100%">';
$html .= '<tr><td>Nombre:</td><td>'.$afavorx.'</td><td align="right">Fecha Solicitud :</td><td>'.$fe2.'</td></tr>';
$html .= '<tr><td>Area :</td><td>'.$detarea.'</td><td align="right">No de Solicitud :</td><td>'.$norden.'</td></tr>';
...
$html .= '<tr><td align="center">Itm</td><td align="center">N/Codigo</td><td align="center">Descripcion</td><td align="center">U.Med</td><td align="center">Cant. Solic.</td><td align="center">Prioridad</td><td align="center">Proveedor</td><td align="center">Precio Unit.</td><td align="center">Subtotal</td></tr>';
$it=0;
$topre=0;
$subtt=0;
$fi = mysqli_query($conexion,"SELECT * FROM solicituddet2 WHERE norden='$norden'");

 while ($row22 = mysqli_fetch_array($fi)) {
 $it=$it+1;
 $nota="--";
 $subtt=$row22['cantsoli']*$row22['preciodt'];
 $topre+=$subtt;
 ...
 $html .= '<tr><td WIDTH="5%" align="center">'.$it.'</td><td WIDTH="10%" align="right">'.$row22['codigo'].'</td><td WIDTH="30%">'.$row22['detrepdt'].'</td><td WIDTH="5%" align="center">'.$row22['umed'].'</td><td WIDTH="10%" align="center">'.$row22['cantsoli'].'</td><td WIDTH="10%" align="center">'.$nota.'</td><td WIDTH="10%" align="center"></td><td WIDTH="10%" align="right">'.$row22['preciodt'].'</td><td WIDTH="10%" align="right">'.number_format($subtt,2).'</td>';
 $html .= '</tr>';
 }
 $html .= '<tr><td colspan="8"  width="30%" align="right">Total Bs.-</td><td align="right">'.number_format($topre,2).'</td>';
$html .= '</table>';
