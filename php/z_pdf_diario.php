...
$html .= '<tr><th>Nro</th><th>&nbsp;Fecha&nbsp;</th><th>&nbsp;Nombre del Cliente&nbsp;</th><th>No Orden</th><th>Tipo</th><th>Importe</th></tr>';
$it=0;
$ttot=0.00;
$row = mysqli_query($conexion,"SELECT * FROM movgymtot AS mv JOIN clientes AS cli ON mv.idcli=cli.idcli WHERE fechamv BETWEEN '$desde' AND '$hasta'");
while ($row2 = mysqli_fetch_array($row)) {
$it=$it+1;
$ttot=$ttot+$row2['importe'];
$ff = date('d/m/Y', strtotime($row2['fechamv']));
...
<td WIDTH="5%" align="center">'.$row2['nordengym'].'</td><td WIDTH="5%" align="center">'.$row2['idtip'].'</td><td WIDTH="8%" align="right">'.$row2['importe'].'</td>';
//$html .= '<td WIDTH="40%"><img src='.$row2['foto'].'  width="250" height="250" </td>';
$html .= '</tr>';

}
 $html .='<tr><td colspan="4"></td><td> Total Bs.- </td><td align="right">'.number_format($ttot,2).'</td><tr>';
$html .= '</table>';
