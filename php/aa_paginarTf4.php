...
//$nordx=0;
//$re2= mysqli_query($conexion,"SELECT * FROM articulos WHERE codigo='$codigoxx' AND codven='$idperx' ");
//while($rev = mysqli_fetch_array($re2)){
//	$codarrx=$rev['codar'];
//	$nordx=$nordx+1;
//}
$nrox = mysqli_num_rows(mysqli_query($conexion,"SELECT * FROM articulos WHERE codigo='$codigoxx' AND codven='$idperx' "));
...
if ($nrox>0){
			mysqli_query($conexion,"UPDATE articulos SET saldo =saldo+'$cantx' WHERE codigo='$codigoxx' AND codven='$idperx'"); //aumentar saldo porke eexiste
	}else{
	/// por si no existe el registro en sucursal se lo crea
	
		mysqli_query($conexion,"INSERT INTO articulos (codigo, descripar, umed, pnetoar, pvpar,feingar,codsuc,saldo,codcla,codpv,observart,stockmin,fotoar,qrfotoar,codven)VALUES
		('$codigoxx','$descriparx','$umedx', '$pnetoarx', '$pvparx', '$fecha','$codsucx','$cantx','$codclax','$codpvx','$observartx','$stockminx','$fotoarx','$qrfotoarx','$idperx')");
...
mysqli_query($conexion,"INSERT INTO movimdet (codar,codigo,descripdt,umed,cant,precio,subtot,norden,fechadt,tipo,codsuc,tmm)VALUES
		('$codarx','$codigoxx','$descriparx', '$umedx', '$cantx', '$pnetoarx','1','$nnox','$fechatox','I','$codsucx','ConfirVendedor')");
...
$registro= mysqli_query($conexion,"SELECT * FROM transferdet WHERE norden='$nordx' AND cdt='' LIMIT $limit, $nroLotes");
    ...
    $tabla = $tabla.'</table>';
	$tabla=$tabla. '<a href="aa_confirmSuc3.php"><img src="../recursos/salida.png" data-toggle="tooltip" title="Cerrar"> Cerrar</a>'.'  ';
