...
$(function(){
	...
	$('#idux').on('change',function(){
		var dato = $('#idux').val();
		var url = '../php/aa_busca_vta1.php';
		$.ajax({
		type:'POST',
		url:url,
		data:'dato='+dato,
...
function VerDocs(id){
	var url = '../php/crm_verPedidos23.php';
	//var pregunta = confirm('¿Esta seguro de eliminar este Registro ?');
	//if(pregunta==true){
		$.ajax({
		type:'POST',
...
//	var hasta = $('#bd-hasta').val();
	window.open('../php/aa_pdf_Ventas.php?nordenx='+nordenx);
}
...
function pagination(partida){
	var url = '../php/crm_paginarPedido2.php';
	$.ajax({
		type:'POST',
		url:url,
