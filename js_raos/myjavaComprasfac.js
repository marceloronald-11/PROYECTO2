...
//$("#mtt").val(res);
	...
	//	}
		...
		if(producto_id!=0){
			...
			//if(cantidad<100000){
				$.ajax({
					url: '../Controller/ControladorVentas2.php?page=1',
					type: 'post',
	...
	$(".guardar-compra").off("click");
	$(".guardar-compra").on("click", function(e) {
		var respo = $('#respo').val();
		var idusu = $('#idusu').val();
...
//		}
		$.ajax({
			url: '../Controller/ControladorVentas2.php?page=3',
			type: 'post',
			data: {'idusu':idusu,'respo':respo,'fechav':fechav,'cpv':cpvx},
			dataType: 'json',
			success: function(data) {
...
window.open('../php/aa_pdf_Compras.php?nordenx='+norden, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=70,left=500,width=700,height=600");
