...
$(function(){
				...
				$('#desart').autocomplete({
                   ...
                   select : function(event, ui){
					...
					$('#respoxxxx').autocomplete({
                   source : 'ajaxactivo.php?type=4',
                   select : function(event, ui){
					   			$('#codpvx').val(ui.item.codpvx);
								$( "#respo" ).focus();
                   }
                });
...
//$("#mtt").val(res);
	...
	//	}
		...
		if(producto_id!=0){
			...
			//if(cantidad<100000){
				$.ajax({
					url: '../Controller/ControladorEnvios2.php?page=1',
					type: 'post',
					data: {'producto_id':producto_id, 'cantidad':cantidad, 'pneto':pneto, 'feven':feven},
					dataType: 'json',
					success: function(data) {
							//var pregunta = confirm('¿lugar 2........ ?');
						if(data.success==true){
							...
							$(".detalle-producto").load('../php/aa_detalle_envio.php');
	...
	$(".guardar-envio").off("click");
	$(".guardar-envio").on("click", function(e) {
		var respo = $('#respo').val();
		var idusu = $('#idusu').val();
		var fechav = $('#fecha').val();
		var codsuc3 = $('#codsuc3').val();
//		var colab = $('#colab').val();
...
//		}
		$.ajax({
			url: '../Controller/ControladorEnvios2.php?page=3',
			type: 'post',
			data: {'idusu':idusu,'respo':respo,'fechav':fechav,'codsucev':codsuc3},
			dataType: 'json',
			success: function(data) {
				...
				//	$(".detalle-producto1").load('../php/2_detalle_ventaumsa.php');
					$(location).attr('href','far_envios.php');
					//location.reload();
				}else{
					alertify.error(data.msj);
				}
...
window.open('../php/aa_pdf_boletaenvio.php?norden='+norden, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=70,left=500,width=700,height=600");
