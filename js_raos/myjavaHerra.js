...
$(function(){
	...
	$('#mqhora').on('keyup',function(){
		var mqh = $('#mqhora').val();

		var rmm=(Number(mqh) /60) ;
		ree = rmm.toFixed(6);
		$('#mqmin').val(ree);
		return false;
	});	
...
function agregaRegistroHe(){
	var url = '../php/jc_agrega_herra.php';
	$.ajax({
		type:'POST',
		url:url,
...
function pagination(partida){
	var url = '../php/jc_paginarHerra.php';
	$.ajax({
		type:'POST',
		url:url,
