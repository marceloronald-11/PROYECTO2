...
//		var hasta = $('#bd-hasta').val();
	...
	$('#bs-prod').on('change',function(){ //sisadal
		...
		var url = '../php/aa_busca_Provv.php';
		$.ajax({
		type:'POST',
		url:url,
		data:'dato='+dato,
...
function agregaRegistroPv(){ // sisadal
	var url = '../php/aa_agrega_Pvv.php';
	$.ajax({
		type:'POST',
		url:url,
...
function editarProvvs(id){ /// sisadal
//var pregunta = confirm('¿Esta seguro de eliminar este !!!!!!');
	$('#formulario')[0].reset();
	var url = '../php/aa_edita_provv.php';
		$.ajax({
		type:'POST',
		url:url,
		data:'id='+id,
...
function eliminarProvv(id){ //SISADAL
	var url = '../php/aa_elimina_Provv.php';
...
function pagination(partida){ /// sidsadal
	var url = '../php/aa_paginarProvv.php';
	$.ajax({
		type:'POST',
		url:url,
