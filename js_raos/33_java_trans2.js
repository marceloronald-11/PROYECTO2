...
$(function(){
				...
				$('#buscaract').autocomplete({
                   source : '1_ajaxart.php?type=1',
                   select : function(event, ui){
						
	
                      // $('#resultados').slideUp('slow', function(){
                         //   $('#resultados').html(
                          //      '<table border="1" class="table" width="80%"><tr><td width="20%"><h4>Vista del Articulo</h4>' +
						//		'<img height="100" width="100" src="' + ui.item.foto + '" />' +'<br/> </td><td align="left" width="30%">' +
							//	'<strong><br>Observacion :</strong>' + ui.item.observ+ '<br>'+
						//		'<strong>Saldo Actual:</strong>' + ui.item.existencia+'</td></tr></table>'
								
								//$(#cantidad).value()=1;
                         //   );
                       		//var producto = ui.item.codser;
					   			$('#idart').val(ui.item.codart);
								$('#cantidad').val('1');
								$('#saldo').val(ui.item.canact);
								$('#umed').val(ui.item.umed);
								$('#ppvp').val(ui.item.precioarti);
								
								$( "#cantidad" ).focus();

								//$('#id-prod').val('7');
							//$('#resultados').slideUp(30000);
					  // });
	...
	$(".btn-agregar-producto").on("click", function(e) {
		...
		var producto_id = $("#codar").val();
		var pventa = $("#pn").val();
		var saldo = Number($("#saldoex").val());
	//	var dcto = $("#dcto").val();
				...
				$.ajax({
					...
					success: function(data) {
						if(data.success==true){
							...
							$(".detalle-producto").load('../php/33_detalle_trans.php');
							//location.reload();
...
if(adepto!=0){
		$.ajax({
			...
			type: 'post',
			data: {'observ':observ,'nnota':nnota,'idusu':idusu,'clasemov':clasem,'adepto':adepto,'fecha':fecha},
			dataType: 'json',
			success: function(data) {
				if(data.success==true){
					location.reload();
			...
			alertify.error('Elija un Departamento ...!');
				//$('#formulario')[0].reset();
				$( "#descrip" ).focus();
...
//							$("#ppvp").val('');
							alertify.success(data.msj);
							$(".detalle-producto").load('../php/22_detalle_infooo.php');
							$('#formulario')[0].reset();
							$( "#descrip" ).focus();
	...
	$(".guardaPerso").off("click");
	$(".guardaPerso").on("click", function(e) {
		var nnota = $('#nnota').val();
...
//		var dote = $('#dote').val();
		...
		$.ajax({
			url: '../Controller/Controlador.php?page=444',
			type: 'post',
...
window.open('../php/z_pdf_repinve.php?norden='+norden, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=70,left=500,width=700,height=600");
