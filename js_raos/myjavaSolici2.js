...
$(function(){
	...
	$('#lineaBorrarxx').on('click',function(){
		var pregunta = confirm('¿Esta seguro de eliminar este Registro ?');
    });



	$(".lineaBorrar").off("click");
	$(".lineaBorrar").on("click", function(e) {
		var pregunta = confirm('¿Anulando......?');
		var id = $(this).attr("id");
		$.ajax({
			...
			if(data.success==true){
				...
				$("#agrega-solicitud").load('../php/jc_detalleSolici2.php');
			}else{
...
//		var saldo =Number( $("#saldo").val()); // codigo del servicio
	...
	$("#btn-GuardaSoli").on("click", function(e) {
		...
		$.ajax({
			url: '../Controller/Controllerjca2.php?page=34',
			type: 'post',
...
function mostrarSolicitud(id){
	window.open('../php/jc_pdf_soli2.php?idd='+id, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=70,left=500,width=500,height=600");
	//location.reload();
}
