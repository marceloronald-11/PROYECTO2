...
function agregaRegistroPago(){
	var url = '../php/aa_agrega_pagosx.php';
	$.ajax({
		type:'POST',
		url:url,
...
function PagandoCre(codcrex){
	$('#formulario')[0].reset();
	var url = '../php/aa_edita_pagoss2.php';
		$.ajax({
		type:'POST',
		url:url,
		...
		success: function(valores){
				...
				$('#ncliente').val(datos[7]);
				$('#impto').val(datos[0]);
				$('#sal').val(datos[1]);
				//$('#nosucc').val(datos[8]);
				$('#nordenxx').val(datos[6]);
...
function VerPaguos(codcrexxx){
	...
	var url = '../php/aa_paginarPagos.php';
	$.ajax({
		type:'POST',
		url:url,
...
function pagination(partida){
	var url = '../php/aa_paginarPagosCreve.php';
	$.ajax({
		type:'POST',
		url:url,
