...
function envia24(norden){
	var url = '../php/jc_compra242.php';
	var pregunta = confirm('¿Esta seguro de cerrar la Nota ?');
	if(pregunta==true){
		$.ajax({
		type:'POST',
		url:url,
...
function envia25(codd,norden){
	var url = '../php/jc_compra2424.php';
	var pregunta = confirm('¿Confirmar Registro ?');
	if(pregunta==true){
		$.ajax({
		type:'POST',
		url:url,
		//data:'id='+norden,
		data:'codd='+codd+'&nordenx='+norden,
		success: function(registro){
			//$('#agrega-registros').html(registro);
			location.reload();
			return false;
...
function Verotro(norden){
	var url = '../php/jc_compra2422.php';
	//var pregunta = confirm('¿Esta seguro de cerrar la Nota ?');
	//if(pregunta==true){
		$.ajax({
		type:'POST',
...
function pagination(partida){
	var url = '../php/jc_paginarEntCom22.php';
	$.ajax({
		type:'POST',
		url:url,
