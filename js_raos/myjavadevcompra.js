...
$(function(){
	...
	$('.fef').on('change', function(){
		...
		var desde = $('.fei').val();
		var hasta = $('.fef').val();
		var url = '../php/crm_fechaCompras.php';
		$.ajax({
		type:'POST',
		url:url,
...
function agregaRegistroCli(){
	var url = '../php/crm_agrega_devcompras.php';
	$.ajax({
		type:'POST',
		url:url,
...
function VerDocs(id){
	var url = '../php/crm_verCompras.php';
	//var pregunta = confirm('¿Esta seguro de eliminar este Registro ?');
	//if(pregunta==true){
		$.ajax({
		type:'POST',
...
function editarCli(id){ //codt
	$('#formulario')[0].reset();
	var url = '../php/crm_editaCompra.php';
		$.ajax({
		type:'POST',
		url:url,
		data:'id='+id,
...
//	var hasta = $('#bd-hasta').val();
	window.open('../php/aa_pdf_Compras.php?nordenx='+nordenx);
}
...
function pagination(partida){
	var url = '../php/crm_paginarDevcompra.php';
	$.ajax({
		type:'POST',
		url:url,
