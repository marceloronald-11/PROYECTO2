...
$(function(){
	...
	$(".btn-agregar-ote").on("click", function(e) {
		//var pregunta = confirm('¿Esta seguro .Ingreso 1..?');
...
//		var saldo =Number( $("#saldo").val()); // codigo del servicio
    ...
    $('#ootes').autocomplete({
     ...
     $("#btn-GuardaVale").on("click", function(e) {
         ...
         var tcax = $('#tca').val();
         var feax = $('#fea').val();
		 var codotex = $('#codotex').val();
         var codigotecx = $('#codigotecx').val();
         var notec = $('#notec').val();
         var fesal = $('#fesal').val();

         
         $.ajax({
             url: '../Controller/Controllerjca4.php?page=2',
             type: 'post',
             data: {'nordenx':nordenx,'tcax':tcax,'feax':feax,'codote':codotex,'codigotec':codigotecx,'notec':notec,'fesal':fesal},
             dataType: 'json',
             success: function(data) {
                 if(data.success==true){
                 //	$("#nomb").val('');
...
function mostrarSolicitud(id){
	window.open('../php/jc_pdf_vales.php?idd='+id, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=70,left=500,width=500,height=600");
	//location.reload();
}
