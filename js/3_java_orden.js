...
$(function(){
				...
				$('#nomtec').autocomplete({
                   source : 'ajaxotes.php?type=1',
                   select : function(event, ui){
                       		//var producto = ui.item.codser;
					   			$('#codtec').val(ui.item.cod_tec);
								//$('#cantidad').val('1');
                   }
                });
	...
	$(".btnTecnico").off("click");
	$(".btnTecnico").on("click", function(e) {
		var codar = $("#codar").val();
	...
	//	if(codotee!=0){
			if(cant!=0){
				$.ajax({
					...
					url: '../Controller/Controlador.php?page=111',
					type: 'post',
					data: {'codar':codar, 'descrip':descrip, 'cod':cod, 'ume':ume, 'pn':pn, 'pv':pv, 'stm':stm, 'observ':observ, 'cla':cla, 'coprov':coprov,'cant':cant},
					dataType: 'json',
					success: function(data) {
...
$( ".LimpiaForm" ).click(function() {
	...
	$(".guardaring").off("click");
	$(".guardaring").on("click", function(e) {
		var nnota = $('#nnota').val();
...
//		var dote = $('#dote').val();
		...
		$.ajax({
			url: '../Controller/Controlador.php?page=222',
			type: 'post',
